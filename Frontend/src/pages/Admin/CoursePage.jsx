...
const CoursePage = () => {
    ...
    return (
        <div className="flex flex-col lg:flex-row bg-green-50 p-4 lg:space-x-10">
        <div className="lg:w-[58%] mb-4 lg:mb-0 lg:ml-[6%]"> {/* Media player section with increased width and 6% left margin */}
            {/* Aspect ratio wrapper with increased size */}
            <div className="relative w-full" style={{ paddingTop: '56.25%' }}> {/* Closer to 16:9 aspect ratio */}
                <ReactPlayer
        ...
        <div className="lg:w-1/4 rounded-lg shadow-lg p-4" style={{ height: 'calc(56.25vw * 0.5625)' }}> {/* Videos list */}
            <h2 className="text-lg font-bold mb-2">Videos</h2>
            <ul className="space-y-6 overflow-y-auto h-full"> {/* Increased vertical gap to 1.5rem */}
                {videos.map((video) => (
                    <li
                        key={video._id}
                        className="cursor-pointer p-2 hover:bg-gray-200 rounded" // Added hover effect without background color
                        onClick={() => handleVideoSelect(video)}
                        style={{ height: '80px' }} // Increased height of list elements
                    >
                        <div className="flex items-center">
                            <img 
                                src={video.thumbnail} 
                                alt={video.title} 
                                className="w-24 h-16 rounded-md mr-3" // Increased size of thumbnails
                            />
                            <div>
                                <span className="font-semibold text-md">{video.title}</span> {/* Adjusted text size */}
                                <div className="text-sm text-gray-600">{video.createdBy?.name}</div> {/* Adjusted text size */}
                            </div>
                        </div>
                    </li>
                ))}
